...
module.exports = {
  ...
  create: Components.detect((context, components) => {
    ...
    const ignoreDefaultParams = configuration.ignoreDefaultParams || false;
     ...
     * @return {void}
     */
    function reportPropTypesWithoutDefault(propTypes, defaultProps, defaultParams) {
      ...
      const allDefaultProps = Object.assign({}, defaultProps, defaultParams);

      Object.keys(propTypes).forEach((propName) => {
        const prop = propTypes[propName];
        if (prop.isRequired) {
          if (forbidDefaultForRequired && allDefaultProps[propName]) {
            context.report({
              node: prop.node,
    ...
    return {
      'Program:exit'() {
        ...
        Object.keys(list).filter(component => list[component].declaredPropTypes).forEach((component) => {
          let defaultParams = {};
          if (list[component].node.params &&
            list[component].node.params[0] &&
            Array.isArray(list[component].node.params[0].properties)
          ) {
            defaultParams = list[component].node.params[0].properties
              .filter(property => property.value.type === 'AssignmentPattern')
              .map(property => ([property.key.name, property]))
          ...
          reportPropTypesWithoutDefault(
            ...
            ignoreDefaultParams ? defaultParams : {}
          );
        });
      }
    };
